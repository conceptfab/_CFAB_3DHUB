...
from PyQt6.QtWidgets import (
    ...
    QPushButton, QLineEdit, QLabel, QFileDialog, QPlainTextEdit, QMessageBox
)
...
class SuffixRemoverApp(QMainWindow):
    ...
    def start_processing(self):
        ...
        self.log_output.clear()
        self.log_output.appendPlainText(f"Rozpoczynam skanowanie folderu: {folder_path}")
                ...
                if os.path.isdir(item_path) and item_name.endswith(suffix_to_remove):
                    ...
                    try:
                        if os.path.exists(new_path):
                            self.log_output.appendPlainText(f"--> [BŁĄD] Folder o nazwie '{new_name}' już istnieje. Pomijam.")
                            continue

                        os.rename(item_path, new_path)
                        self.log_output.appendPlainText(f"--> [SUKCES] Zmieniono nazwę na: '{new_name}'")
                        changed_count += 1
                    except PermissionError:
                        self.log_output.appendPlainText(f"--> [BŁĄD] Brak uprawnień do zmiany nazwy folderu '{item_name}'.")
                    except Exception as e:
                        self.log_output.appendPlainText(f"--> [BŁĄD KRYTYCZNY] Wystąpił nieoczekiwany błąd: {e}")
                
        except FileNotFoundError:
            self.log_output.appendPlainText(f"[BŁĄD] Folder '{folder_path}' nie został znaleziony.")
        except Exception as e:
            self.log_output.appendPlainText(f"[BŁĄD KRYTYCZNY] Wystąpił nieoczekiwany błąd podczas skanowania: {e}")
        
        self.log_output.appendPlainText("-" * 30)
        self.log_output.appendPlainText(f"Zakończono. Zmieniono nazwy {changed_count} folderów.")
