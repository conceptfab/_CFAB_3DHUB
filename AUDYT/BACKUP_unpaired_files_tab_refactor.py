...
class UnpairedFilesTab:
    ...
    def update_unpaired_files_lists(self):
        ...
        if not hasattr(self, "unpaired_archives_list_widget") and not hasattr(
            self, "unpaired_archives_list"
        ):
            logging.error("CRITICAL ERROR: Brak widget'a do zarządzania archiwami!")
            return

        if not hasattr(self, "unpaired_previews_list_widget") and not hasattr(
            self, "unpaired_previews_grid"
        ):
            logging.error("CRITICAL ERROR: Brak widget'a do zarządzania podglądami!")
            return

        if not hasattr(self, "unpaired_previews_layout") and not hasattr(
            self, "unpaired_previews_grid"
        ):
            logging.error(
        ...
        if (
            self.unpaired_archives_list_widget is None
            and self.unpaired_archives_list is None
        ):
            logging.error("Brak widget'a do zarządzania archiwami!")
            return

        if (
            self.unpaired_previews_list_widget is None
            and self.unpaired_previews_grid is None
        ):
