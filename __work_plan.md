...
*   **Dokumentacja:**
    *   `[ ] Do uzupełnienia`: `README.md` - instrukcja instalacji i uruchomienia.
    *   `[ ] Do uzupełnienia`: Podstawowe komentarze i docstringi w utworzonych plikach.
...
*   **Wymagane Funkcjonalności:**
    ...
    3.  **`src/logic/metadata_manager.py` (nowy):**
        ...
        *   Funkcja `save_metadata(working_directory, file_pairs_list)`:
            ...
            *   Przygotowuje dane do zapisu: słownik, gdzie kluczem jest *względna ścieżka pliku archiwum* (względem `working_directory`), a wartością jest słownik atrybutów `FilePair` do zapisania (np. `is_favorite`, później gwiazdki, kolory). **Nie zapisujemy miniatur w JSON.**
