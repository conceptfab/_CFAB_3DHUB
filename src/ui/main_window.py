...
class MainWindow(QMainWindow):
    ...
    def _show_archive_context_menu(self, position):
        ...
        open_action = QAction("Otwórz w domyślnym programie", self)
    ...
    def _open_archive_externally(self, item):
        ...
        if not os.path.exists(archive_path):
            QMessageBox.warning(
                self, "Plik nie istnieje", f"Plik {archive_path} nie istnieje."
            )
            return
    ...
    def _add_preview_thumbnail(self, preview_path):
        ...
        # Przechowaj ścieżkę pliku w danych widgetu
    ...
    def _delete_preview_file(self, preview_path):
        ...
        if not os.path.exists(preview_path):
            QMessageBox.warning(
                self, "Plik nie istnieje", f"Plik {preview_path} nie istnieje."
            )
            return
    ...
    def _refresh_unpaired_files(self):
        ...
        current_folder = self.current_working_directory
        if current_folder and os.path.isdir(current_folder):
            # Pobierz aktualne pary plików, niesparowane archiwa i podglądy
            found_pairs, unpaired_archives, unpaired_previews = (
                self.directory_tree_manager.scan_folder_for_file_pairs(current_folder)
            )

            # Zapisz nowe dane
            self.file_pairs = found_pairs
            self.unpaired_archives = unpaired_archives
            self.unpaired_previews = unpaired_previews
