    ...
    def mouseMoveEvent(self, event):
        ...
        if (
            ...
            if target_widget_at_press not in [
                ...
                logging.debug(
                    f"Rozpoczęcie przeciągania dla: {self.file_pair.base_name}"
                )
                ...
                if drop_action == Qt.DropAction.MoveAction:
                    logging.debug(
                        f"Operacja przeciągania zakończona sukcesem dla {self.file_pair.base_name}"
                    )
                else:
                    logging.debug(
                        f"Operacja przeciągania anulowana dla {self.file_pair.base_name}"
                    )
            ...
            if self.maybe_drag and not self.drag_active and self.press_pos is not None:
                ...
                if (
                    ...
                    if clicked_widget == self.thumbnail_label:
                        if self.file_pair:
                            logging.debug(
                                f"Kliknięcie na miniaturę: {self.file_pair.base_name}"
                            )
                            self.preview_image_requested.emit(self.file_pair)
                        self.maybe_drag = False
                        self.press_pos = None
                        ...
                        if self.file_pair:
                            logging.debug(
                                f"Kliknięcie na nazwę pliku: {self.file_pair.base_name}"
                            )
                            self.archive_open_requested.emit(self.file_pair)
                        self.maybe_drag = False
                        self.press_pos = None
    ...
    def _create_context_menu(self) -> QMenu:
        ...
        if self.file_pair:
            action_open.triggered.connect(
                lambda: self.archive_open_requested.emit(self.file_pair)
            )
            action_preview.triggered.connect(
                lambda: self.preview_image_requested.emit(self.file_pair)
            )
