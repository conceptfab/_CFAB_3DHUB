"""
🚨 ETAP 2 - POPRAWKA 5: FileExplorerTab - Trzecia zakładka z eksploratorem plików
"""
...
class FileExplorerTab(QWidget):
    ...
    def _create_directory_tree(self):
        ...
        tools_widget.setStyleSheet("QWidget { background-color: #252526; border: none; }")
        ...
        tools_label.setStyleSheet("font-weight: bold; font-size: 16px; color: #CCCCCC; margin-bottom: 8px; padding: 8px; background-color: transparent; border: none;")
        tools_layout.addWidget(tools_label)
        
        # Przycisk do renumeratora duplikatów
        self.duplicate_renamer_button = QPushButton("🎯 Uruchom Renumerator Duplikatów")
        self.duplicate_renamer_button.setToolTip("Znajdź i zmień nazwy duplikatów plików w aktualnym folderze")
        ...
        self.refresh_files_button.setToolTip("Odśwież listę plików w aktualnym folderze")
        ...
        info_label.setStyleSheet("font-weight: bold; color: #CCCCCC; margin-bottom: 6px; font-size: 14px;")
    ...
    def _create_files_panel(self):
        ...
        files_widget.setStyleSheet("QWidget { background-color: #252526; border: none; }")
    ...
    def set_root_path(self, path):
        ...
        try:
            ...
            if len(path) > 80:
                ...
                if len(parts) > 3:
                    display_folder = f"{parts[0]}{os.sep}...{os.sep}{parts[-2]}{os.sep}{parts[-1]}"
                else:
                    display_folder = path
            else:
                display_folder = path
    ...
    def _launch_duplicate_renamer(self):
        """Uruchamia narzędzie renumeratora duplikatów."""
        try:
            ...
            # Uruchom renumerator z wybranym folderem
            import subprocess
            import sys
            
            script_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "random_name.py")
            
            if not os.path.exists(script_path):
                QMessageBox.critical(
                    self,
                    "Błąd",
                    f"Nie znaleziono skryptu renumeratora:\n{script_path}"
                )
                return
            
            # Uruchom w osobnym procesie
            subprocess.Popen([sys.executable, script_path])
            
            logging.info(f"Launched duplicate renamer: {script_path}")
            
            # Pokaż informację
            QMessageBox.information(
                self,
                "Renumerator uruchomiony",
                f"Uruchomiono renumerator duplikatów.\n\nAktualny folder roboczy:\n{self.current_path}\n\nWybierz ten folder w oknie renumeratora."
            )
            ...
            QMessageBox.critical(
                self,
                "Błąd",
                f"Błąd podczas uruchamiania renumeratora:\n{e}"
            ) 