...
class ResourceLimits:
    ...
    max_memory_per_tile_mb: int = 5
    max_concurrent_workers: int = 10
    cleanup_interval_seconds: int = 300  # 5 minut
    memory_check_interval_seconds: int = 60  # 1 minuta
    cache_cleanup_threshold_ratio: float = 0.8  # Cleanup gdy cache 80% pełny
...
class MemoryMonitor(QObject):
    ...
    def _check_memory_usage(self):
        ...
        try:
            ...
            # Sprawdź limity
            warning_threshold = self.limits.max_memory_mb * 0.8
            critical_threshold = self.limits.max_memory_mb * 0.95
...
class WorkerPoolManager:
    ...
    def register_worker(self) -> Optional[int]:
        ...
        with self._lock:
            ...
            logger.debug(f"Registered worker {worker_id}, active: {len(self._active_workers)}")
            return worker_id
    ...
    def unregister_worker(self, worker_id: int):
        ...
        with self._lock:
            ...
            logger.debug(f"Unregistered worker {worker_id}, active: {len(self._active_workers)}")
...
class TileResourceManager(QObject):
    ...
    def _initialize_performance_components(self):
        ...
        try:
            ...
            self._cache_optimizer = get_cache_optimizer(max_size_mb=self.limits.max_memory_mb * 0.3)
            self._async_ui_manager = get_async_ui_manager(max_concurrent_tasks=self.limits.max_concurrent_workers)
            
            # Connect signals
            if self._performance_monitor:
                self._performance_monitor.performance_alert.connect(
                    lambda metric, value: self.resource_warning.emit(f"Performance alert: {metric} = {value}")
                )
                
            if self._cache_optimizer:
                self._cache_optimizer.memory_pressure_detected.connect(
                    lambda usage_mb: self._on_memory_warning(f"Cache memory pressure", usage_mb)
                )
                
            if self._async_ui_manager:
                self._async_ui_manager.performance_warning.connect(
                    lambda warning_type, value: self.resource_warning.emit(f"UI performance: {warning_type} = {value}")
                )
    ...
    def enable_performance_optimization(self, enabled: bool = True):
        ...
        if enabled and not (self._performance_monitor and self._cache_optimizer and self._async_ui_manager):
            self._initialize_performance_components()
        elif not enabled:
    ...
    def register_tile(self, tile) -> bool:
        ...
        with self._lock:
            if len(self._active_tiles) >= self.limits.max_tiles:
                ...
                self.resource_warning.emit(f"Tile limit reached: {self.limits.max_tiles}")
                return False
    ...
    def perform_cleanup(self):
        ...
        with self._lock:
            ...
            logger.debug(f"Cleanup completed in {cleanup_time:.3f}s, cleaned: {cleaned_count}")
        ...
        with self._lock:
            ...
            if self._cache_optimizer:
                try:
                    ...
                    stats.cache_hits = int(total_operations * stats.cache_hit_ratio / 100)
    ...
    def get_statistics(self) -> MemoryUsageStats:
        ...
        if self._stats_history:
            ...
            self._update_statistics()
            return self._stats_history[-1] if self._stats_history else MemoryUsageStats()
    ...
    def is_resource_limit_reached(self) -> bool:
        ...
        return (
            stats.total_tiles >= self.limits.max_tiles or
            stats.total_memory_mb >= self.limits.max_memory_mb or
            stats.memory_per_tile_mb >= self.limits.max_memory_per_tile_mb
        )
    ...
    def get_debug_info(self) -> Dict:
        ...
        return {
            ...
            'component_counts': {
                name: len(comp_set) for name, comp_set in self._active_components.items()
            },
...
def get_resource_manager(limits: Optional[ResourceLimits] = None) -> TileResourceManager:
