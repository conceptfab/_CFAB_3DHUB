...
class UIUpdateScheduler(QObject):
    """Scheduler dla UI updates z priorytetami (heapq + QTimer)."""

    task_scheduled = pyqtSignal(str)
    task_completed = pyqtSignal(str, float)
    memory_pressure = pyqtSignal(int)  # queue_size

    def __init__(self, max_concurrent: int = 3, max_queue_size: int = 500):
        super().__init__()
        self.max_concurrent = max_concurrent
        self.max_queue_size = max_queue_size
        self._task_queue = []  # heapq
    ...
    def schedule_task(self, task: UIUpdateTask) -> bool:
        """Planuje zadanie UI update (heapq, limit kolejki)."""
        with self._lock:
            if len(self._task_queue) >= self.max_queue_size:
                self.memory_pressure.emit(len(self._task_queue))
                logger.warning(
                    f"Task queue full ({self.max_queue_size}), task rejected!"
                )
                return False
            # heapq: (priority, created_time, task)
            heappush(self._task_queue, (task.priority.value, task.created_time, task))
            self.task_scheduled.emit(f"Priority_{task.priority.name}")
            return True

    def process_tasks(self):
        """Przetwarza zadania z kolejki (heapq, QTimer)."""
        with self._lock:
            while self._task_queue and len(self._active_tasks) < self.max_concurrent:
                _, _, task = heappop(self._task_queue)
                task_id = id(task)
...
class DebounceManager(QObject):
    ...
    def debounce(
        self, operation_id: str, func: Callable, delay_ms: int = 100, *args, **kwargs
    ):
        """Debounce operację."""
        with self._lock:
            # Store the operation
            ...
            timer = threading.Timer(
                delay_seconds, self._execute_debounced, [operation_id]
            )
            timer.daemon = True
            timer.start()
...
class TileAsyncUIManager(QObject):
    ...
    def __init__(self, max_concurrent_tasks: int = 5, enable_batching: bool = True):
        ...
        logger.info(
            f"TileAsyncUIManager initialized "
            f"(concurrent_tasks: {max_concurrent_tasks}, "
    ...
    def schedule_thumbnail_load(
        self, load_func: Callable, *args, callback: Optional[Callable] = None, **kwargs
    ):
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "batch_queue_size": (
                len(self._batch_updater._batch_queue) if self._batch_updater else 0
            ),
        }
