...
class FolderOperationsManager:
    ...
    def create_folder(self, parent_folder_path: str, callback: Optional[Callable] = None):
        ...
        progress_dialog = self._create_progress_dialog("Tworzenie folderu...", folder_name)
    ...
    def rename_folder(self, folder_path: str, callback: Optional[Callable] = None):
        ...
        # Utwórz progress dialog
        progress_dialog = self._create_progress_dialog("Przemianowywanie folderu...", f"{current_name} → {new_name}")
        ...
        def on_finished(old_path, new_path):
            ...
            QMessageBox.information(
                self.parent_window,
                "Sukces",
                f"Folder przemianowany na '{new_name}'"
            )
        ...
        # Podłącz sygnały
        worker.signals.finished.connect(lambda: on_finished(folder_path, new_folder_path))
        worker.signals.error.connect(on_error)
        worker.signals.progress.connect(on_progress)
    ...
    def delete_folder(self, folder_path: str, callback: Optional[Callable] = None):
        ...
        # Utwórz progress dialog
        progress_dialog = self._create_progress_dialog("Usuwanie folderu...", folder_name)
        ...
        def on_finished(deleted_path):
            ...
            QMessageBox.information(
                self.parent_window,
                "Sukces",
                f"Folder '{folder_name}' został usunięty!"
            )
