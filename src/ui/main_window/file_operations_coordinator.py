...
class FileOperationsCoordinator:
    ...
    def perform_bulk_move(self):
        ...
        self.main_window.progress_manager.show_progress(
            0, f"Przenoszenie {selected_count} par plików..."
        )
    ...
    def handle_file_drop_on_folder(
        ...
        self.logger.info(
            f"Drag & drop: {len(source_file_paths)} plików do {target_folder_path}"
        )
        ...
        if not os.path.exists(target_folder_path) or not os.path.isdir(
            ...
            # USUNIĘTO: Okno błędu - drag & drop powinien działać cicho
    ...
    def on_bulk_move_files_finished(self, result):
        ...
        # USUNIĘTO: Raport po drag & drop - ma działać cicho
    ...
    def show_detailed_move_report(
        ...
        report_lines.append(
            f"Przeniesiono pomyślnie: {summary.get('moved_count', 0)} par plików"
        )
    ...
    def show_detailed_files_move_report(
        ...
        report_lines.append(
            f"Przeniesiono pomyślnie: {summary.get('successfully_moved', 0)} plików"
        )
