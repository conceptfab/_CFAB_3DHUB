...
class ViewRefreshManager(QObject):
    ...
    def _is_refresh_needed(self, view_name: str, current_state: Dict[str, Any]) -> bool:
        ...
        for key in view_def["state_keys"]:
            if current_state.get(key) != last_state.get(key):
                logger.debug(f"Zmiana w {view_name}.{key}: {last_state.get(key)} -> {current_state.get(key)}")
                return True
    ...
    def _get_gallery_state(self) -> Dict[str, Any]:
        ...
        if hasattr(self.main_window, 'filter_panel'):
            try:
                current_filter = self.main_window.filter_panel.get_current_filter()
            except:
                pass
                
        # Sprawdź liczbę plików
        if hasattr(self.main_window, 'controller') and hasattr(self.main_window.controller, 'current_file_pairs'):
    ...
    def _get_unpaired_files_state(self) -> Dict[str, Any]:
        ...
        if hasattr(self.main_window.controller, 'unpaired_archives'):
            unpaired_archives = list(self.main_window.controller.unpaired_archives or [])
        if hasattr(self.main_window.controller, 'unpaired_previews'):
            unpaired_previews = list(self.main_window.controller.unpaired_previews or [])
    ...
    def _get_metadata_state(self) -> Dict[str, Any]:
        ...
        return {
            "folder": current_folder,
            "metadata_modified_time": modified_time
        }
    ...
    def _perform_scheduled_refreshes(self):
        ...
        logger.debug(f"Wykonuję odświeżenie widoków: {', '.join(self._pending_refreshes)}")
    ...
    def _refresh_view_immediately(self, view_name: str):
        ...
        try:
            ...
            if hasattr(self, refresh_method_name):
                ...
                logger.warning(f"Brak metody {refresh_method_name} dla widoku {view_name}")
                success = False
            ...
            logger.debug(f"Odświeżenie {view_name} zakończone w {elapsed:.3f}s (sukces: {success})")
